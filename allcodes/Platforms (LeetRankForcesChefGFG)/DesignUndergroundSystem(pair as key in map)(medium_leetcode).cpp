...
*/
// storing pair -> String as key is new thing to know
// 100%
...
// using pair as key requires custom has function definition
// my soln.
class UndergroundSystem {
    unordered_map<int, pair<string, int>> travel;
    map<string, map<string, pair<int, int>>> record;
public:
    UndergroundSystem() {
        int t = 0;
    }
    ...
    void checkOut(int id, string stationName, int t) {
        pair<string, int> detail = travel[id];
        string startStation = detail.first;
        int startTime = detail.second;
        
        // pair p = make_pair(startStation, stationName);
        pair<int, int> prevRecord = record[startStation][stationName];
        record[startStation][stationName] = {prevRecord.first + t - startTime, prevRecord.second + 1};
    }
    
    double getAverageTime(string startStation, string endStation) {
        // pair p = make_pair(startStation, endStation);
        
        return 1.0 * record[startStation][endStation].first / record[startStation][endStation].second;
    }
};
