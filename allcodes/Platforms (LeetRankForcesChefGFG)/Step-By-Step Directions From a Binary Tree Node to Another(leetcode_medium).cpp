...
public:
    TreeNode* lca(TreeNode* root, int startValue, int destValue) {
        ...
        if(root->val == startValue || root->val == destValue)
            return root;
        TreeNode* leftAns = lca(root->left, startValue, destValue);
        TreeNode* rightAns = lca(root->right, startValue, destValue);
        if(leftAns && rightAns)
            return root;
        return leftAns == NULL ? rightAns : leftAns;
    }
    int getHeight(TreeNode* root, int n) {
        if(!root)
            return -1;
        if(root->val == n)
            return 0;
    ...
    bool getPathString(TreeNode* root, int n, string &path) {
        if(!root)
            return false;
        if(root->val == n)
            return true;
        ...
        bool found = getPathString(root->left, n, path);
        if(found)
            return true;
        path.pop_back();
        
        path.push_back('R');            
        found = getPathString(root->right, n, path);
        if(found)
            return true;
    ...
    string getDirections(TreeNode* root, int startValue, int destValue) {
        ...
        int h = getHeight(lcaNode, startValue);
        string rightAns = "";
        getPathString(lcaNode, destValue, rightAns);
        
        return string(h, 'U') + rightAns;
    }
};