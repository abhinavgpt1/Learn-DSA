...
// Solution 4 - conventional way DFS
class Solution
{
    bool isCycle(vector<int> adj[], int index, vector<bool> &visited, vector<bool> &pathVisited)
    {
        ...
        for (int v : adj[index])
        {
            if (!visited[v])
            {
                if (isCycle(adj, v, visited, pathVisited))
                    return true;
            }
...
public:
    ...
    bool isCyclic(int V, vector<int> adj[])
    {
        ...
        // conventional way for cycle detection in directed graph by DFS
        vector<bool> visited(V, false);
        vector<bool> pathVisited(V, false);
        for (int i = 0; i < V; i++)
        {
            if (!visited[i] && isCycle(adj, i, visited, pathVisited))
                return true;
        }
        return false;
    }
