...
pair<TreeNode<int>*, TreeNode<int>*> helper(TreeNode<int>* root){
    ...
    for(int i=0; i<root->children.size(); i++){
        pair<TreeNode<int>*, TreeNode<int>*> smallAns = helper(root->children[i]);
        if(smallAns.first->data > ans.first->data){
            if(smallAns.second != NULL && smallAns.second->data > ans.first->data){
        ...
        else if(smallAns.first->data == ans.first->data){
            if(ans.second == NULL)
                ans.second = smallAns.second;
            else if(smallAns.second != NULL && smallAns.second->data > ans.second->data)
                ans.second = smallAns.second;
        }
        else{
          if(ans.second == NULL)
            ans.second = smallAns.first;
        else if(smallAns.first->data > ans.second->data)
            ans.second = smallAns.first;  
        } 
    }
    return ans;
